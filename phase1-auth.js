...
function parseRoomFromPath(pathname) {
  ...
  // Match /room/:modelname or /room/:modelname/:roomslug (legacy)
...
class Phase1Auth {
  ...
  async initiateJoinRequest() {
    ...
    // Update debug panel
    if (typeof updateDebugJoinStatus === 'function') {
      updateDebugJoinStatus('requesting');
    }

    try {
      // Extract creator slug and room slug from URL path using utility
      const urlPath = window.location.pathname;
      
      // Try /r/:creatorSlug/:roomSlug pattern first
      let parsed = parseCreatorSlugFromPath(urlPath);
      let creatorSlug = parsed.creatorSlug;
      let roomSlugParam = parsed.roomSlug;
      
      // Fall back to legacy /room/:modelname/:roomslug pattern
      if (!creatorSlug) {
      ...
      if (!creatorSlug) {
        throw new Error('Invalid room URL - could not parse creator slug');
      }
      ...
      if (response.ok) {
        ...
        // Update debug panel
        if (document.getElementById('debug-request-id')) {
          document.getElementById('debug-request-id').textContent = data.requestId;
        }
        if (typeof updateDebugJoinStatus === 'function') {
          updateDebugJoinStatus('pending');
        }
        ...
        // Update debug panel
        if (typeof updateDebugError === 'function') {
          updateDebugError(data.error || 'Join request failed');
        }
        if (typeof updateDebugJoinStatus === 'function') {
          updateDebugJoinStatus('error: ' + (data.error || 'unknown'));
        }
      ...
      // Update debug panel
      if (typeof updateDebugError === 'function') {
        updateDebugError(error.message || 'Join request error');
      }
      if (typeof updateDebugJoinStatus === 'function') {
        updateDebugJoinStatus('error: ' + error.message);
      }
    ...
    try {
      ...
      if (response.ok) {
        if (data.status === 'approved' && data.dailyToken) {
          ...
          // Update debug panel
          if (typeof updateDebugJoinStatus === 'function') {
            updateDebugJoinStatus('approved');
          }
